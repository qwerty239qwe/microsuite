...
def metadata_tabulate(
    *, backend: str, input_file: Path | None, output: Path | None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    _run(
        [qiime, "metadata", "tabulate", "--m-input-file", str(input_file),
         "--o-visualization", str(output)],
        "QIIME 2 metadata tabulate failed.",
        run_dir, timeout, "metadata_tabulate", backend,
    )


def qiime_import(
    *, backend: str, input_path: Path | None, output: Path | None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    _run(
        [qiime, "tools", "import", "--type", "EMPSingleEndSequences",
         "--input-path", str(input_path), "--output-path", str(output)],
        "QIIME 2 EMP single-end import failed.",
        run_dir, timeout, "qiime_import", backend,
    )


def demux(
    *, backend: str, seqs: Path | None, metadata: Path | None, barcode_column: str | None,
    output_demux: Path | None, output_details: Path | None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    _run(
        [qiime, "demux", "emp-single", "--i-seqs", str(seqs), "--m-barcodes-file",
         str(metadata), "--m-barcodes-column", barcode_column, "--o-per-sample-sequences",
         str(output_demux), "--o-error-correction-details", str(output_details)],
        "QIIME 2 EMP single-end demultiplexing failed.",
        run_dir, timeout, "demux", backend,
    )


def feature_summarize(
    *, backend: str, mode: str, table: Path | None = None, rep_seqs: Path | None = None,
    metadata: Path | None = None, output: Path | None = None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    if mode == "summarize":
        ...
        command = [qiime, "feature-table", "tabulate-seqs", "--i-data", str(rep_seqs),
                   "--o-visualization", str(output)]
    else:
        raise MicrobiomeSuiteError("--mode must be 'summarize' or 'tabulate-seqs'.")
    _run(command, "QIIME 2 feature-table visualization failed.", run_dir, timeout,
         "feature_summarize", backend)


def phylogeny(
    *, backend: str, rep_seqs: Path | None, output_aligned: Path | None,
    output_masked: Path | None, output_tree: Path | None, output_rooted_tree: Path | None,
    threads: int | str = 1, force: bool = False, run_dir: Path | None = None,
    timeout: float | None = None,
) -> None:
    _expect_backend("phylogeny", backend)
    rep_seqs = _required(rep_seqs, "--rep-seqs", backend)
    outputs = [_required(path, flag, backend) for path, flag in [
        (output_aligned, "--output-aligned"), (output_masked, "--output-masked"),
        (output_tree, "--output-tree"), (output_rooted_tree, "--output-rooted-tree")]]
    qiime = require_qiime("QIIME 2 MAFFT/FastTree phylogeny")
    ensure_input(rep_seqs)
    for path in outputs:
        prepare_output(path, force=force)
    command = [qiime, "phylogeny", "align-to-tree-mafft-fasttree", "--i-sequences",
               str(rep_seqs), "--p-n-threads", str(resolve_threads(threads)),
               "--o-alignment", str(output_aligned), "--o-masked-alignment",
               str(output_masked), "--o-tree", str(output_tree), "--o-rooted-tree",
               str(output_rooted_tree)]
    _run(command, "QIIME 2 phylogeny failed.", run_dir, timeout, "phylogeny", backend)


def diversity_core(
    *, backend: str, table: Path | None, phylogeny_path: Path | None, metadata: Path | None,
    sampling_depth: int, output_dir: Path | None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    _run([qiime, "diversity", "core-metrics-phylogenetic", "--i-phylogeny",
          str(phylogeny_path), "--i-table", str(table), "--p-sampling-depth",
          str(sampling_depth), "--m-metadata-file", str(metadata), "--output-dir",
          str(output_dir)], "QIIME 2 core metrics phylogenetic failed.", run_dir,
         timeout, "diversity_core", backend)


def diversity_test(
    *, backend: str, alpha_diversity: Path | None = None, distance_matrix: Path | None = None,
    metadata: Path | None = None, metadata_column: str | None = None,
    output: Path | None = None, method: str = "permanova", pairwise: bool = False,
    force: bool = False, run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    if backend == "qiime2-alpha-group-significance":
        ...
        command = [qiime, "diversity", "alpha-group-significance", "--i-alpha-diversity",
                   str(alpha_diversity), "--m-metadata-file", str(metadata),
                   "--o-visualization", str(output)]
    else:
        ...
        command = [qiime, "diversity", "beta-group-significance", "--i-distance-matrix",
                   str(distance_matrix), "--m-metadata-file", str(metadata),
                   "--m-metadata-column", metadata_column, "--p-method", method,
                   "--o-visualization", str(output)]
        command.append("--p-pairwise" if pairwise else "--p-no-pairwise")
    _run(command, "QIIME 2 diversity group significance failed.", run_dir, timeout,
         "diversity_test", backend)


def ordination_plot(
    *, backend: str, pcoa: Path | None, metadata: Path | None, output: Path | None,
    force: bool = False, run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    ensure_input(metadata)
    prepare_output(output, force=force)
    _run([qiime, "emperor", "plot", "--i-pcoa", str(pcoa), "--m-metadata-file",
          str(metadata), "--o-visualization", str(output)], "QIIME 2 Emperor plot failed.",
         run_dir, timeout, "ordination_plot", backend)


def rarefaction(
    *, backend: str, table: Path | None, phylogeny_path: Path | None, metadata: Path | None,
    max_depth: int, output: Path | None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    prepare_output(output, force=force)
    command = [qiime, "diversity", "alpha-rarefaction", "--i-table", str(table),
               "--p-max-depth", str(max_depth), "--m-metadata-file", str(metadata)]
    if phylogeny_path is not None:
        command.extend(["--i-phylogeny", str(phylogeny_path)])
    command.extend(["--o-visualization", str(output)])
    _run(command, "QIIME 2 alpha rarefaction failed.", run_dir, timeout,
         "rarefaction", backend)


def tax_train(
    *, backend: str, ref_seqs: Path | None, ref_taxonomy: Path | None, f_primer: str | None,
    r_primer: str | None, trunc_len: int, output: Path | None, threads: int | str = 1,
    force: bool = False, run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    first = [qiime, "feature-classifier", "extract-reads", "--i-sequences", str(ref_seqs),
             "--p-f-primer", f_primer, "--p-r-primer", r_primer, "--p-trunc-len",
             str(trunc_len), "--p-n-jobs", threads_value, "--o-reads", str(reads)]
    second = [qiime, "feature-classifier", "fit-classifier-naive-bayes",
              "--i-reference-reads", str(reads), "--i-reference-taxonomy",
              str(ref_taxonomy), "--o-classifier", str(output)]
    if run_dir is None:
        run_qiime(first, "QIIME 2 extract-reads failed.", timeout=timeout,
                  task="tax_train", backend=backend)
        run_qiime(second, "QIIME 2 fit-classifier-naive-bayes failed.", timeout=timeout,
                  task="tax_train", backend=backend)
    else:
        run_qiime(first, "QIIME 2 extract-reads failed.", run_dir=run_dir / "extract-reads",
                  timeout=timeout, task="tax_train", backend=backend)
        run_qiime(second, "QIIME 2 fit-classifier-naive-bayes failed.",
                  run_dir=run_dir / "fit-classifier-naive-bayes", timeout=timeout,
                  task="tax_train", backend=backend)
        _write_summary_run(run_dir, "tax_train", backend, [first, second], {"classifier": output})


def tax_barplot(
    *, backend: str, table: Path | None, taxonomy: Path | None, metadata: Path | None,
    output: Path | None, force: bool = False, run_dir: Path | None = None,
    timeout: float | None = None,
) -> None:
    ...
    for path in (table, taxonomy, metadata):
        ensure_input(path)
    prepare_output(output, force=force)
    _run([qiime, "taxa", "barplot", "--i-table", str(table), "--i-taxonomy",
          str(taxonomy), "--m-metadata-file", str(metadata), "--o-visualization",
          str(output)], "QIIME 2 taxa barplot failed.", run_dir, timeout, "tax_barplot",
         backend)


def feature_filter(
    *, backend: str, table: Path | None, metadata: Path | None, where: str | None,
    output: Path | None, force: bool = False, run_dir: Path | None = None,
    timeout: float | None = None,
) -> None:
    ...
    ensure_input(metadata)
    prepare_output(output, force=force)
    command = [qiime, "feature-table", "filter-samples", "--i-table", str(table),
               "--m-metadata-file", str(metadata), "--o-filtered-table", str(output)]
    if where:
...
def tax_collapse(
    *, backend: str, table: Path | None, taxonomy: Path | None, level: int,
    output: Path | None, force: bool = False, run_dir: Path | None = None,
    timeout: float | None = None,
) -> None:
    ...
    prepare_output(output, force=force)
    _run([qiime, "taxa", "collapse", "--i-table", str(table), "--i-taxonomy",
          str(taxonomy), "--p-level", str(level), "--o-collapsed-table", str(output)],
         "QIIME 2 taxa collapse failed.", run_dir, timeout, "tax_collapse", backend)


def diff_viz(
    *, backend: str, data: Path | None, output: Path | None, force: bool = False,
    run_dir: Path | None = None, timeout: float | None = None,
) -> None:
    ...
    ensure_input(data)
    prepare_output(output, force=force)
    _run([qiime, "composition", "da-barplot", "--i-data", str(data),
          "--o-visualization", str(output)], "QIIME 2 DA barplot failed.", run_dir,
         timeout, "diff_viz", backend)
...
def _run(
    ...
    run_qiime(command, failure_message, run_dir=run_dir, timeout=timeout, task=task,
              backend=backend.lower())
