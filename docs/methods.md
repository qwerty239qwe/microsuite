...
| `native-adonis` | microsuite 0.1.0 | ready | `microsuite diversity adonis beta.tsv --metadata metadata.tsv --formula 'd ~ disease_status + accession + accession:timepoint + (1 \| accession:subject_id)' -o adonis.tsv` | `adonis2(distance_matrix, metadata, formula=..., permutations=999)` | [microsuite Python](../containers/microsuite/Dockerfile) | Pure Python, no R or container needed. `--by terms` gives sequential (type I) sums of squares, where order matters for confounded terms; `--by margin` adjusts every term for all others so order does not, and reports a term nested inside another as 0 df. A `(1 \| group)` term restricts permutation rather than fitting a variance component, and unlike `vegan-adonis2` it still runs when group sizes are unbalanced. | Multi-term PERMANOVA with interactions and restricted permutation. |
| `vegan-adonis2` | vegan 2.7.x | ready | `microsuite diversity beta-significance beta.tsv --metadata metadata.tsv --backend vegan --formula "site + phase" --strata subject --method adonis2 --runtime docker` | `vegan_beta_significance(distance_matrix, metadata, formula="site + phase", strata="subject", method="adonis2")` | [R ecology](../containers/r-ecology/Dockerfile) | Formula-aware PERMANOVA with vegan's restricted permutations; requires R/vegan locally or the heavy ecology image, and `strata` permutation requires balanced group sizes. | Vegan adonis2 beta-diversity significance. |
