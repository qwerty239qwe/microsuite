...
| Generate OTU-style table | `mothur` | FASTA sequences with sample IDs in labels | OTU count table | table TSV, representative FASTA, sidecar `.shared` | mothur MiSeq SOP clustering; requires a user-supplied aligned reference. |
...
| `mothur` | mothur 1.48.5 | ready | `microsuite cluster --backend mothur --reference-alignment silva.v4.fasta` | `cluster(backend="mothur", rep_seqs=..., output_table=..., output_rep_seqs=..., reference_alignment=...)` | [mothur](../containers/mothur/Dockerfile) or external `mothur` | Alignment-based OTU clustering with OptiClust across 12 mothur commands; needs a user-supplied aligned reference and is slower than VSEARCH. | mothur MiSeq SOP OTU clustering. |
