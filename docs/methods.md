...
| `native-beta-significance` | microsuite 0.1.0 | ready | `microsuite diversity beta-significance beta.tsv --metadata metadata.tsv --column body_site` | `beta_significance(distance_matrix, metadata, column=..., method="permanova")` | [microsuite Python](../containers/microsuite/Dockerfile) | Unrestricted one-factor PERMANOVA/PERMDISP/ANOSIM; multifactor and repeated-measures designs belong in `native-adonis` or `vegan-adonis2`. | Native beta-diversity location and dispersion tests. |
| `native-adonis` | microsuite 0.3.0 | ready | `microsuite diversity adonis beta.tsv --metadata metadata.tsv --formula 'd ~ batch + group / timepoint + (1 \| batch:subject_id)' -o adonis.tsv` | `adonis2(distance_matrix, metadata, formula=..., permutations=999)` | [microsuite Python](../containers/microsuite/Dockerfile) | Pure Python, no R or container needed. Supports `+`, `:`, `*`, and `/`; `--by terms` gives sequential (type I) sums of squares, while `--by margin` adjusts every term for all others. A `(1 \| group)` term restricts permutation rather than fitting a variance component, and unlike vegan's fixed `strata` blocks it can exchange equal-sized groups. | Multi-term PERMANOVA with interactions, nesting, and restricted permutation. |
| `vegan-adonis2` | vegan 2.7.x | ready | `microsuite diversity beta-significance beta.tsv --metadata metadata.tsv --backend vegan --formula "site + phase" --strata subject --method adonis2 --runtime docker` | `vegan_beta_significance(distance_matrix, metadata, formula="site + phase", strata="subject", method="adonis2")` | [R ecology](../containers/r-ecology/Dockerfile) | Formula-aware PERMANOVA with vegan's restricted permutations; requires R/vegan locally or the heavy ecology image. Strata are fixed blocks and may be unequal, but singleton/coarse designs limit permutation resolution. | Vegan adonis2 beta-diversity significance. |
...
block. Samples shuffle only within each joint level; whole subjects are not
exchanged between batches. It restricts permutations and is not an lme4-style
random effect. All-singleton interactions are rejected, and output distinguishes
requested from effective permutation counts.
...
| `maaslin3` | MaAsLin 3 1.4.0 | ready | `microsuite diff_abundance --backend maaslin3` | `diff_abundance(backend="maaslin3", table=..., formula=..., output=...)` | [R diffab maaslin3](../containers/r-diffab-maaslin3/Dockerfile) | Full lme4 formula support, including random effects; emits distinct abundance and prevalence tables. See [MaAsLin 3](maaslin3.md). | Joint abundance/prevalence multivariable association testing. |
| `lefse` | lefser 1.22.0 | ready | `microsuite diff_abundance --backend lefse` | `diff_abundance(backend="lefse", table=..., group=..., output=...)` | [R diffab lefse](../containers/r-diffab-lefse/Dockerfile) | Reproducible two-class LEfSe with explicit reference, optional crossed subclass/block design, validated thresholds and stable output. See [LEfSe](lefse.md). | LEfSe biomarker discovery. |
...
| Backend | Version | Status | CLI command | Python invocation | Image / environment | Operational tradeoff | Purpose |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `picrust2` | PICRUSt2 2.6.3 | ready | `microsuite functional_profile --backend picrust2 --table table.tsv --rep-seqs rep-seqs.fasta --output-dir functions --picrust2-database SC` | `functional_profile(backend="picrust2", table=..., rep_seqs=..., output_dir=...)` | [Pinned PICRUSt2](../containers/microsuite-picrust2/Dockerfile) or matching local environment | SC is the default; oldIMG and custom single/dual reference sets are selectable. Raw upstream outputs plus a microsuite manifest; pathway coverage is opt-in experimental. | Predict reference-dependent function from marker-gene profiles. |
| `tax4fun2` | Tax4Fun2 1.1.5 | ready | `microsuite functional_profile --backend tax4fun2 --table otu-table.tsv --rep-seqs otus.fasta --database Tax4Fun2_ReferenceData_v2 --output-dir functions --runtime docker` | `functional_profile(backend="tax4fun2", table=..., rep_seqs=..., database=..., output_dir=..., runtime="docker")` | [Pinned Tax4Fun2](../containers/r-functional-tax4fun2/Dockerfile) or exact local 1.1.5 environment | Strict ID/matrix/reference validation, stable prediction and coverage tables, reference fingerprints, and real planted Docker smoke; the large v2 reference remains user-supplied. | Infer reference-dependent functional potential from 16S profiles. |
